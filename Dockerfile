...
RUN apt-get update && apt-get upgrade -y &&     rm -r /var/lib/apt/lists /var/cache/apt/archives
RUN useradd -r -u 1001 -g root kafka

COPY rootfs-kafka /
RUN /postunpack.sh
#ENV BITNAMI_APP_NAME="kafka" #   BITNAMI_IMAGE_VERSION="2.4.0-debian-10-r46" ENV NAMI_PREFIX="/.nami"     PATH="/opt/bitnami/java/bin:/opt/bitnami/common/bin:/opt/bitnami/kafka/bin:/opt/bitnami/gosu/bin:$PATH"

EXPOSE 9092

RUN mv /entrypoint.sh /entrypoint-kafka.sh
RUN mv /setup.sh /setup-kafka.sh
...
ENV BITNAMI_APP_NAME="kafka-zookeeper"     BITNAMI_IMAGE_VERSION="2.4.0-debian-10-r46+3.6.0-debian-10-r2"     PATH="/opt/bitnami/java/bin:/opt/bitnami/zookeeper/bin:/opt/bitnami/common/bin:/opt/bitnami/gosu/bin:$PATH"
#    NAMI_PREFIX="/.nami"
